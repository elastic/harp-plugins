...
var descriptor = &artifact.Command{
	...
	Module:      "cmd/harp-linter",
	Name:        "Harp Linter",
	Description: "Harp Bundle Linter",
}

// Build the artefact
func Build() {
	banner := figure.NewFigure("Harp Linter", "", true)
	banner.Print()
...
func Test() {
	...
	mg.SerialDeps(
		func() error {
			return golang.UnitTest("github.com/elastic/harp-plugins/cmd/harp-linter/pkg/...")()
		},
		func() error {
			return golang.UnitTest("github.com/elastic/harp-plugins/cmd/harp-linter/internal/...")()
		},
	)
}
...
func Compile() error {
	// Extract
	version, err := git.TagMatch("cmd/harp-linter/v*")
	if err != nil {
		return err
	}

	// Build artifact
	return golang.Build("harp-linter", "github.com/elastic/harp-plugins/cmd/harp-linter", version)()
}
	...
	mg.CtxDeps(ctx,
		func() error {
			return golang.Release(
				"harp-linter",
				"github.com/elastic/harp-plugins/cmd/harp-linter",
				version,

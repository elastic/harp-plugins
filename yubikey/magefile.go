...
var descriptor = &artifact.Command{
	Package:     "github.com/elastic/harp-plugins/yubikey",
	Module:      "cmd/harp-yubikey",
	Name:        "Harp Yubikey",
	Description: "Harp Yubikey identity manager",
}

// Build the artefact
func Build() {
	banner := figure.NewFigure("Harp Yubikey", "", true)
	banner.Print()
...
func Test() {
	...
	mg.SerialDeps(
		func() error {
			return golang.UnitTest("github.com/elastic/harp-plugins/yubikey/pkg/...")()
		},
	)
}
...
func Compile() error {
	// Extract
	version, err := git.TagMatch("yubikey/v*")
	if err != nil {
		return err
	}

	// Build artifact
	return golang.Build("harp-yubikey", "github.com/elastic/harp-plugins/yubikey/cmd/harp-yubikey", version, golang.WithCGO())()
}
	...
	mg.CtxDeps(ctx,
		func() error {
			return golang.Release(
				"harp-yubikey",
				"github.com/elastic/harp-plugins/yubikey/cmd/harp-yubikey",
				version,

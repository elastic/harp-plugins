...
var descriptor = &artifact.Command{
	Package:     "github.com/elastic/harp-plugins/assertion",
	Module:      "cmd/harp-assertion",
	Name:        "Harp Assertion",
	Description: "Harp assertion manager",
}

// Build the artefact
func Build() {
	banner := figure.NewFigure("Harp Assertion", "", true)
	banner.Print()
	...
	color.Red("# Pipeline -----------------------------------------------------------------")
	mg.SerialDeps(golang.Vendor, golang.License("../"), Generate, golang.Lint("../"))
...
func Test() {
	...
	mg.SerialDeps(
		func() error {
			return golang.UnitTest("github.com/elastic/harp-plugins/assertion/pkg/...")()
		},
		func() error {
			return golang.UnitTest("github.com/elastic/harp-plugins/assertion/internal/...")()
		},
	)
}
...
func Compile() error {
	// Extract
	version, err := git.TagMatch("assertion/v*")
	if err != nil {
		return err
	}

	// Build artifact
	return golang.Build("harp-assertion", "github.com/elastic/harp-plugins/assertion/cmd/harp-assertion", version)()
}
	...
	mg.CtxDeps(ctx,
		func() error {
			return golang.Release(
				"harp-assertion",
				"github.com/elastic/harp-plugins/assertion/cmd/harp-assertion",
				version,

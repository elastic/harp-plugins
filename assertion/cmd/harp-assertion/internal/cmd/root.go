...
import (
	...
	iconfig "github.com/elastic/harp-plugins/assertion/cmd/harp-assertion/internal/config"
...
var mainCmd = func() *cobra.Command {
	...
	cmd.AddCommand(configcmd.NewConfigCommand(conf, "SHUB_ASSERTION"))

	cmd.AddCommand(signCmd(conf))
	cmd.AddCommand(verifyCmd(conf))
	cmd.AddCommand(jwksCmd(conf))
...
func initConfig() {
	if err := config.Load(conf, "HARP_ASSERTION", cfgFile); err != nil {
